...
namespace STR_CRL_API_COMALM.BL
{
    public class Sq_Rendicion
    {
        ...
        */
        ...
        {
            var respIncorrect = "no se puede registrar Documento";
            Sq_Item sq = new Sq_Item();
            List<Complemento> list = new List<Complemento>();

            try
            {
                hash.insertValueSql(SQ_QueryManager.Generar(Sq_Query.pos_rendicionedoc),
            doc.STR_RENDICION,
            ...
            string idDoc = hash.GetValueSql(SQ_QueryManager.Generar(Sq_Query.get_idDOC), doc.STR_RD_ID.ToString());
                ...
                //hash.insertValueSql(SQ_QueryManager.Generar(Sq_Query.pos_rendiciones), e.STR_CODARTICULO?.id,
                //    e.STR_CODARTICULO!= null?sq.ObtieneItems("art", e.STR_ALMACEN).Result[0].ItemName : null,
                //    e.STR_SUBTOTAL, e.STR_INDIC_IMPUESTO.id, e.STR_DIM1.name, e.STR_DIM2.name, e.STR_DIM4.name, e.STR_DIM5,
                //   e.STR_CANTIDAD, e.STR_TPO_OPERACION, idDoc);
